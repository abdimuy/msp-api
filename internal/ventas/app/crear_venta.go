...
//
//  1. Validates that every producto (including combo children) has sufficient
//     existencia in its origin almacén BEFORE the venta is persisted. The
//     validation runs in its own READ COMMITTED NO WAIT transaction so
//     simultaneous "last item" sales fail fast with
//     apperror.NewValidation("articulo_sin_existencia").
...
func (s *Service) CrearVenta(ctx context.Context, in CrearVentaInput, by uuid.UUID) (*domain.Venta, error) {
	...
	// Build the domain aggregate first (pure, no side effects).
	...
	// Now validate stock using the full venta (includes combo children).
...
// venta (including combo children) and calls validateStockParaDetalles.
// Returns nil when s.inventario is nil.
func (s *Service) validateStockFromVenta(ctx context.Context, venta *domain.Venta) error {
	if s.inventario == nil {
		return nil
	}
	...
	return s.validateStockParaDetalles(ctx, detalles, almacenOrigen)
}

// crearTraspasoParaVenta emits the automatic traspaso that reserves the
// venta's productos (including combo children) in the configured destino
// almacén. Skips when no InventarioService is wired or when the venta has no
// productos.
//
// The detalles and shared AlmacenOrigen are derived from the fully built venta
// via buildTraspasoDetallesFromVenta — combo children inherit their origin from
// the parent combo, so they are included in the reservation.
func (s *Service) crearTraspasoParaVenta(ctx context.Context, venta *domain.Venta, by uuid.UUID, now time.Time) error {
	if s.inventario == nil {
		return nil
	}
	detalles, almacenOrigen, err := buildTraspasoDetallesFromVenta(venta)
