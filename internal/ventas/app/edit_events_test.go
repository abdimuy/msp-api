...
func TestCrearVenta_ClienteIDInvalidoPropagates(t *testing.T) {
	t.Parallel()
	h := newHarness(t)
	h.svc = ventasapp.NewService(h.ventas, newFakeClienteChecker(false), nil, h.storage, h.clock, h.outbox, h.imageProc, nil, nil, nil, nil)
...
func TestCrearVenta_ClienteIDValidoPersiste(t *testing.T) {
	t.Parallel()
	h := newHarness(t)
	h.svc = ventasapp.NewService(h.ventas, newFakeClienteChecker(true), nil, h.storage, h.clock, h.outbox, h.imageProc, nil, nil, nil, nil)
	in := validContadoInput()
	cid := 42
	in.ClienteID = &cid
...
func TestCrearVenta_NilClienteIDBypassesChecker(t *testing.T) {
	...
	h.svc = ventasapp.NewService(h.ventas, checker, nil, h.storage, h.clock, h.outbox, h.imageProc, nil, nil, nil, nil)
	in := validContadoInput()
	in.ClienteID = nil
	_, err := h.svc.CrearVenta(t.Context(), in, uuid.New())
	require.NoError(t, err)
