...
func (w *VentaWriter) Aplicar(ctx context.Context, in outbound.MicrosipVentaInput) (outbound.MicrosipVentaResult, error) {
	...
	wc := firebird.ToWallClock(fechaVenta)
	hora := firebird.ToWallClock(fechaVenta)
	...
	if v.TipoVenta() == domain.TipoVentaCredito {
		if err := w.insertDatosCredito(ctx, q, in, v, doctoPVID, *v.ClienteID(), claveCliente, in.SucursalID, totalConIVA, fechaVenta); err != nil {
