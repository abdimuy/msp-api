// Package invoutbox is the inventario module's adapter to the platform outbox.
//
// Per docs/adr/0001-outbox-strategy.md the inventario module's enqueuer is
...
// Enqueuer wraps the platform outbox so the inventario module can drop events
// on the queue without owning a *pgxpool.Pool reference directly.
...
func (e *Enqueuer) Enqueue(ctx context.Context, aggregate string, aggregateID uuid.UUID, eventType string, payload any) error {
	...
	if err != nil {
		...
		slog.ErrorContext(ctx, "inventario.outbox_enqueue_failed",
			"aggregate", aggregate,
