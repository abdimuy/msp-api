...
// StorageKind enum values matching MSP_PAGOS_IMAGENES.STORAGE_KIND.
//
...
// Allowed image and document MIME types matching MSP_PAGOS_IMAGENES.MIME.
const (
	...
	MimeWebP = "image/webp"
	// MimePDF is the canonical mime for PDF files (recibos de transferencia
	// bancaria suelen subirse como PDF).
	MimePDF = "application/pdf"
)

// IsAllowedMime reports whether m is one of the allowed image/PDF mime types.
func IsAllowedMime(m string) bool {
	switch m {
	case MimeJPEG, MimePNG, MimeGIF, MimeWebP, MimePDF:
		return true
	}
	return false
}
