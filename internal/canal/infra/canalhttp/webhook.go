...
// A missing or invalid signature is 403 with no body. A valid signature is
// always 200, even if the message inside turns out to be malformed or
// persistence fails: this is a deliberate deviation from strict REST
// status semantics — see the package doc and task-6-report.md — because
// returning a non-2xx here only teaches Meta to retry a delivery that
// either already landed (harmless: RecibirMensaje is idempotent by wamid)
// or never will (a body persistence keeps failing on will keep failing).
// Failures past the signature check are logged instead.
func (h *webhookHandler) handleReceive(w http.ResponseWriter, r *http.Request) {
	...
	h.procesarEntrantes(r.Context(), body)
	w.WriteHeader(http.StatusOK)
}
...
// inbound message it finds to the service. Parse and persistence failures
// are logged, never returned — the caller has already committed to
// answering 200 regardless (see handleReceive's doc comment).
func (h *webhookHandler) procesarEntrantes(ctx context.Context, body []byte) {
	...
	for _, entry := range payload.Entry {
		for _, change := range entry.Changes {
			for _, msg := range change.Value.Messages {
				h.procesarMensaje(ctx, change.Value.Metadata.PhoneNumberID, msg)
			}
		}
	}
}
...
// hands it to the service. A malformed timestamp or a service error is
// logged and the message is dropped — see procesarEntrantes.
func (h *webhookHandler) procesarMensaje(ctx context.Context, phoneNumberID string, msg wireMessage) {
	tipo := msg.str("type")
	wamid := msg.str("id")
	...
	if _, err := h.svc.RecibirMensaje(ctx, params); err != nil {
